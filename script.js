...
function applyTimeGreeting() {
  ...
  if (h >= 0 && h < 6) {
    const display = h === 0 ? 'เที่ยงคืน' : `ตี ${h}`;
    ...
    const display = h === 6 ? '6 โมงเช้า' : `${h} โมงเช้า`;
    timeLabel = `สวัสดีช่วง${display}ครับ`;
    ...
    timeLabel = 'สวัสดีช่วง 6 โมงเย็นครับ';
    msg = `เลิกงานแล้วยังมาดูโปรไฟล์ผม\nแสดงว่าเรื่องนี้ไม่ใช่แค่งาน routine สำหรับคุณ\nมาเริ่มกันเลยครับ`;
  } else if (h >= 19 && h <= 21) {
    timeLabel = `สวัสดีช่วง ${h - 18} ทุ่มครับ`;
    msg = `เลิกงานแล้วยังมาดูโปรไฟล์ผม\nแสดงว่าเรื่องนี้ไม่ใช่แค่งาน routine สำหรับคุณ\nมาเริ่มกันเลยครับ`;
  } else {
    const display = h === 22 ? '4 ทุ่ม' : '5 ทุ่ม';
    timeLabel = `สวัสดีช่วง${display}ครับ`;
    msg = `ดึกขนาดนี้แล้วยังเข้ามาดูโปรไฟล์ผม\nแสดงว่าตำแหน่งนี้ต้องสำคัญกับคุณมากแน่ๆ\nถ้าอย่างนั้นมาเริ่มกันเลยครับ`;
  }
...
function buildInsight() {
  ...
  return insightMap[top] || `คุณหยุดอ่านนานที่สุดที่ "${top}" — นั่นบอกผมบางอย่างแล้วครับ`;
}
